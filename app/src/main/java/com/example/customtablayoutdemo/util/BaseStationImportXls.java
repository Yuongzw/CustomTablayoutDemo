...
public class BaseStationImportXls extends BaseStationImportBase {
	...
	protected void importFile(File file) throws Exception {
		...
		for (int i = 0; i < mWorkbook.getNumberOfSheets(); i++) {
			...
			for (int r = firstRowIndex + 1; r < lastRowIndex; r++) {
				...
				if(this.isColsHasValue(rows, COLNAME.SSB_ARFCN)||this.isColsHasValue(rows, COLNAME.Center_ARFCN)||this.isColsHasValue(rows, COLNAME.NARFCN)) {
					netType = BaseStation.NETTYPE_ENDC;
				}else if (this.isColsHasValue(rows, COLNAME.ENBID_LCELLID)) {
				...
				} else if (this.isColsHasValue(rows, COLNAME.G_NODEB, COLNAME.G_NB, COLNAME.NSA, COLNAME.SA, COLNAME.SSB, COLNAME.NCI)){
                    netType = BaseStation.NETTYPE_ENDC;
                }
				if (netType == -1)
					continue;
	...
	private void createBaseStationDetail(BaseStation base, String[] values, Map<COLNAME, Integer> colMap, int netType) {
		...
		switch (netType) {
			...
			case BaseStation.NETTYPE_ENDC:
				detail.pci = this.getStringValue(values, colMap, COLNAME.PCI);
				detail.earfcn = getEndcEarfcn(values,colMap);//this.getStringValue(values, colMap, COLNAME.NARFCN);
				detail.earfcn = getEndcEarfcn(values,colMap);//this.getStringValue(values, colMap, COLNAME.NARFCN);
				detail.enodebIp = this.getStringValue(values, colMap, COLNAME.ENODEBIP);
				detail.sectorId = this.getStringValue(values, colMap, COLNAME.SECTORID);
				detail.azimuth = this.getStringValue(values, colMap, COLNAME.AZIMUTH);
	...
	private String getEndcEarfcn(String[] values,Map<COLNAME, Integer> colMap){
		String earfcn=this.getStringValue(values, colMap, COLNAME.NARFCN);
		if(null==earfcn||earfcn.equals("")){
			earfcn=this.getStringValue(values, colMap, COLNAME.Center_ARFCN);
			if(null==earfcn||earfcn.equals("")){
				earfcn=this.getStringValue(values, colMap, COLNAME.SSB_ARFCN);
			}
		}
		return earfcn;
	}
	...
	private void getColMap(String[] colNames) {
		...
		this.checkColName(COLNAME.SITENAME, colNames, "SITE NAME","Cell Name","gNodeB Name");
		...
		this.checkColName(COLNAME.SSB_ARFCN, colNames, "SSB ARFCN");
		this.checkColName(COLNAME.Center_ARFCN, colNames, "Center ARFCN");
		this.checkColName(COLNAME.NARFCN, colNames, "NARFCN");
		this.checkColName(COLNAME.GNODEBID, colNames, "gNodeB ID", "gNB ID");
        this.checkColName(COLNAME.G_NODEB, colNames, "gNodeB");
        this.checkColName(COLNAME.G_NB, colNames, "gNB");
        this.checkColName(COLNAME.NSA, colNames, "NSA");
        this.checkColName(COLNAME.SA, colNames, "SA");
        this.checkColName(COLNAME.SSB, colNames, "SSB");
        this.checkColName(COLNAME.NCI, colNames, "NCI");
		this.checkColName(COLNAME.TAC, colNames, "TAC");
		this.checkColName(COLNAME.SITEID, colNames, "SITE ID");
	}
